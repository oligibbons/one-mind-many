...
// MERGED: This version is from File 2. It correctly deletes from
// game_players first to avoid foreign key constraints.
router.delete('/games/:id', isAdmin, async (req, res) => {
  try {
    ...
    if (playerError) {
      console.error('Delete game (players) error:', playerError);
      throw playerError;
    }
    ...
    if (error) {
      console.error('Delete game (game) error:', error);
      throw error;
    }
...
// This is the original, correct GET route from File 1
...
// MERGED: Added from File 2 for full CRUD
...
router.post('/scenario', isAdmin, async (req, res) => {
  ...
  // TODO: Add validation for scenarioData
  try {
    const { data, error } = await supabase
      .from('scenarios')
      .insert(scenarioData)
...
router.put('/scenario/:id', isAdmin, async (req, res) => {
  const { id } = req.params;
  const scenarioData = req.body;
  delete scenarioData.id; // Don't try to update the primary key
...
// MERGED: Added from File 2 for full CRUD (with game check)
router.delete('/scenario/:id', isAdmin, async (req, res) => {
  const { id } = req.params;
  try {
    // Check if any games are using this scenario
