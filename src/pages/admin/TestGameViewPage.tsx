...
import { ArrowLeft, Play, RotateCcw, Eye, Users, Clock, Trophy } from 'lucide-react';
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center">
          ...
          <div>
            <h1 className="text-3xl font-bold text-white">Test Game Environment</h1>
      ...
      {viewMode === 'game' && gameState && (
        <TestGameInterface gameState={gameState} />
      )}
...
const TestGameInterface = ({ gameState }: { gameState: TestGameState }) => {
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-6">
            <div className="text-white">
              ...
              <span className="ml-2 text-xl font-bold">{gameState.current_state.round}</span>
            </div>
            <div className="text-white">
              <span className="text-sm text-slate-400">Turn</span>
              <span className="ml-2 text-xl font-bold">{gameState.current_state.turn}</span>
            </div>
            <div className="text-white">
              <span className="text-sm text-slate-400">Phase</span>
              <span className="ml-2 text-lg font-medium capitalize">{gameState.current_state.phase}</span>
            </div>
          </div>
    ...
    <div className="space-y-8">
      ...
      <Card className="p-8 text-center">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          <div className="text-center">
            <div className="text-3xl font-bold text-white">{postGameState.results.duration}m</div>
            <div className="text-slate-400">Duration</div>
          </div>
          <div className="text-center">
            <div className="text-3xl font-bold text-white">{postGameState.results.turns_completed}</div>
          ...
          <div className="text-center">
            <div className="text-3xl font-bold text-white">{postGameState.players.length}</div>
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <div className="text-right">
                  <div className="text-lg font-bold text-white">{player.successful_actions}/{player.actions_taken}</div>
          ...
          {postGameState.timeline.map((event, index) => (
            <div key={index} className="flex items-start">
              <div className="w-8 h-8 bg-orange-500 rounded-full flex items-center justify-center mr-4 mt-1">
                <span className="text-white text-sm font-bold">{event.turn}</span>
              </div>
              <div>
      ...
      <Card className="p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <div className="space-y-3">
              <div className="flex justify-between">
                <span className="text-slate-400">Most Active Player:</span>
                <span className="text-white">{postGameState.statistics.most_active_player}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-400">Most Successful Player:</span>
                <span className="text-white">{postGameState.statistics.most_successful_player}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-400">Total Actions:</span>
                <span className="text-white">{postGameState.statistics.total_actions}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-400">Successful Actions:</span>
                <span className="text-white">{postGameState.statistics.successful_actions}</span>
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="space-y-3">
              <div className="flex justify-between">
                <span className="text-slate-400">Sabotage Attempts:</span>
                <span className="text-white">{postGameState.statistics.sabotage_attempts}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-400">Objectives Failed:</span>
                <span className="text-white">{postGameState.statistics.objectives_failed}</span>
              </div>
              <div className="flex justify-between">
