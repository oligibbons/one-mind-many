/*
  ...
  1. New Tables
    - `games` - Core game instances
    - `characters` - Game characters with location and stats
    - `character_inventory` - Items owned by characters
    - `game_actions` - Player actions during games
    - `game_logs` - Game event logs and narratives

  2. Security
    - Enable RLS on all tables
    - Add policies for game participants to access their game data
    - Add policies for players to create actions in their games

  3. Functions
    - `update_character_location` - Update character position
    - `add_character_item` - Add items to character inventory
    - `queue_player_action` - Queue player actions for processing
    - `add_narrative_log` - Add narrative entries to game logs
*/
